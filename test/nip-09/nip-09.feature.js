...
import { createEvent, sendEvent, waitForEventCount } from '../helpers.js'
...
When(/^(\w+) sends a delete event for their last event$/, async function (
  ...
  const tags = [
    ['e', this.parameters.events[name][this.parameters.events[name].length - 1].id]
  ]

  const event = await createEvent({ pubkey, kind: 5, content: '', tags }, privkey)
...
Then(
  ...
  async function (name, count, author) {
    const ws = this.parameters.clients[name]
...
When(/^(\w+) drafts a text_note event with content "([^"]+)"$/, async function (name, content) {
  const { pubkey, privkey } = this.parameters.identities[name]
...
Given(/^(\w+) drafts a set_metadata event$/, async function (name) {
  const { pubkey, privkey } = this.parameters.identities[name]
